...
@app.route("/examens_afficher/<string:order_by>/<int:id_examen_sel>", methods=['GET', 'POST'])
def examens_afficher(order_by, id_examen_sel):
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                if order_by == "ASC" and id_examen_sel == 0:
                    strsql_examen_afficher = """SELECT * FROM t_examen ORDER BY id_examen ASC"""
                    ...
                    valeur_id_examen_selected_dictionnaire = {"value_id_examen_selected": id_examen_sel}
                    strsql_examen_afficher = """SELECT * FROM t_examen WHERE id_examen = %(value_id_examen_selected)s"""

                    mc_afficher.execute(strsql_examen_afficher, valeur_id_examen_selected_dictionnaire)
                else:
                    strsql_examen_afficher = """SELECT * FROM t_examen ORDER BY id_examen DESC"""
        ...
        except Exception as ExceptionExamensAfficher:
            raise ExceptionExamensAfficher(f"fichier : {Path(__file__).name}  ;  "
                                          f"{examens_afficher.__name__} ; "
                                          f"{ExceptionExamensAfficher}")
...
def examen_ajouter_wtf():
    ...
    if request.method == "POST":
        ...
        except Exception as Exception_examens_ajouter_wtf:
            raise ExceptionExamensAjouterWtf(f"fichier : {Path(__file__).name}  ;  "
                                            f"{examen_ajouter_wtf.__name__} ; "
                                            f"{Exception_examens_ajouter_wtf}")
...
@app.route("/examen_update", methods=['GET', 'POST'])
def examen_update_wtf():
    # L'utilisateur vient de cliquer sur le bouton "EDIT". Récupère la valeur de "id_genre"
    id_genre_update = request.values['id_examen_btn_edit_html']
...
@app.route("/examen_delete", methods=['GET', 'POST'])
def examen_delete_wtf():
    data_films_attribue_genre_delete = None
    btn_submit_del = None
    # L'utilisateur vient de cliquer sur le bouton "DELETE". Récupère la valeur de "id_genre"
    id_genre_delete = request.values['id_examen_btn_delete_html']
    ...
    try:
        ...
        if request.method == "POST" and form_delete.validate_on_submit():
            ...
            if form_delete.submit_btn_conf_del.data:
                ...
                # le formulaire "genres/examen_delete_wtf.html" lorsque le bouton "Etes-vous sur d'effacer ?" est cliqué.
        ...
        if request.method == "GET":
            ...
            # Afficher la valeur sélectionnée dans le champ du formulaire "examen_delete_wtf.html"
            form_delete.nom_genre_delete_wtf.data = data_nom_genre["intitule_genre"]

            # Le bouton pour l'action "DELETE" dans le form. "examen_delete_wtf.html" est caché.
